...
> 门控变体（SwiGLU / GeGLU）见 `pytorch.nn.gated_activations`；
> 数值稳定的 sigmoid / softmax 见 `pytorch.nn.numeric_activations`。
...
- 输入是 `torch.float32`。**禁止用** `F.gelu` / `F.silu` 等内置函数，自己按公式实现。
- 容差 `atol=1e-6`。
