...
为了让你专注「拼装」，judge 已经提供了下面两个工具，**请直接 import 使
用**，不要重新实现：
...
SwiGLU FFN 请自己用几个 `nn.Linear` 在 `forward` 里拼出来（见下）。
...
- `self.W_q`, `self.W_k`, `self.W_v`, `self.W_o`: 四个 `nn.Linear(d_model, d_model, bias=False)`
- `self.ffn_norm_weight`: `nn.Parameter(torch.ones(d_model))`  ← FFN 前 RMSNorm
- `self.gate_proj`, `self.up_proj`: `nn.Linear(d_model, d_ff, bias=False)`
- `self.down_proj`: `nn.Linear(d_ff, d_model, bias=False)`

> 说明：RMSNorm 的缩放向量是逐元素参数，用 `nn.Parameter`；四个注意力投影
> 和三个 FFN 投影都是无 bias 的线性层，用 `nn.Linear(..., bias=False)`。
...
def forward(self, x, mask=None):
    ...
    # 投影 + 切多头
    q = self.W_q(h).reshape(B, T, num_heads, head_dim).transpose(1, 2)
    k = self.W_k(h).reshape(B, T, num_heads, head_dim).transpose(1, 2)
    v = self.W_v(h).reshape(B, T, num_heads, head_dim).transpose(1, 2)
    attn_out = sdpa(q, k, v, mask)                                # (B, H, T, d_h)
    attn_out = self.W_o(attn_out.transpose(1, 2).reshape(B, T, D))
    x = x + attn_out

    # 2. FFN sub-block（SwiGLU）
