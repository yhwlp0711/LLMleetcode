...
- **KL 惩罚固定用 k3 估计器**（`exp(logr)-1-logr`，`logr = logp_ref - logp`），逐 token 施加到 reward 后再走 GAE。见 `pytorch.llm.kl_penalty_estimators`。
...
> 无 critic 版（advantage 来自组内 z-score）见 `pytorch.llm.grpo_loss`。
